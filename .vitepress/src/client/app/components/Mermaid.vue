<template>
  <div>
    <div v-html="show ? code: svg" v-if="rendered" @click="show=!show">
    </div>
  </div>
</template>
<script>
    ...
    onMounted(() => {
      import("mermaid/dist/mermaid.min").then(() => {
        ...
        mermaid.mermaidAPI.render(props.id, props.graph, (svg_rendered, ...args) => {
          svg.value = svg_rendered.replace(/xmlns="http:\/\/www.w3.org\/2000\/svg" height="100%" /, `xmlns="http://www.w3.org/2000/svg" `)
        })
      }).catch( error => {
